...
func (c *Cluster) Start() error {
	...
	if isUpgrade {
		...
		logger.Infof("successfully ugraded MDS cluster for filesystem %s", c.fs.Name)
	}
...
func (c *Cluster) scaleDownDeployments(replicas int32, activeCount int32, desiredDeployments map[string]bool, delete bool) error {
	...
	for _, d := range deps.Items {
		if _, ok := desiredDeployments[d.GetName()]; !ok {
			...
			logger.Infof("Deleting or stopping mds deployment %s", d.GetName())
	...
	logger.Infof("successfully deleted or stopped mds deployments outside desired deployments")
